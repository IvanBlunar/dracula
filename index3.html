...
<head>
  ...
  <style>
    ...
    width: 100%;
    max-width: 100%;
    overflow-x: hidden;            /* corta desbordes al documento */
    overscroll-behavior-x: none;   /* sin “arrastre” lateral */
    touch-action: pan-y;           /* solo desplazamiento vertical */
    position: relative;
...
<style>
    ...
    @media (max-width:768px){
      #cta-salir{ bottom: 20px; } /* antes: bottom:20; */
    }
    ...
    .hero-monster{
      position:absolute;
      z-index:26;                  /* por encima de la intro */
      top:7%;
      left:10%;
...
/* Opcionales: atenuar o desactivar si un asset queda muy oscuro */
.blend-soft{ opacity:.9; }
.blend-normal{ mix-blend-mode: normal !important; }
  ...
  .pages-area::after{
    ...
    background-size:contain;   /* respeta proporción del PNG/WebP */
    pointer-events:none;
    z-index:25;                /* debajo de action-bar (28) y héroes (26) */
    mix-blend-mode:multiply;   /* modo de fusión */
    display:none;              /* solo se muestra con data-realm */
  }
...
.animate-soft-blink {
  ...
  <section id="captura-bestias" class="relative z-10 min-h-[100vh] md:min-h-[110vh] flex flex-col md:flex-row-reverse items-center justify-center p-6 overflow-visible">
    ...
    <!-- Texto -->
    <div class="relative z-30 max-w-xl text-white text-center md:text-center p-4 md:p-8 flex flex-col items-center md:items-start">
  ...
  <div class="page-right">
    ...
    <div id="grid-home" class="grid grid-cols-2 gap-3 md:gap-4 place-items-center">
      ...
      <button class="btn-swap icon-btn" data-goto="vampirico" aria-label="Ir al Reino Vampírico">
        ...
        <!-- CTA (reemplazo con tu arte) -->
        <a id="cta-salir" href="#" class="btn-swap cta-swap" aria-label="Salir al bosque">
          <img class="off" src="assets/BT SALIR OFF.webp" alt="Salir al bosque">
...
.icon-grid-tight {
  ...
  column-gap: -10px !important; /* Espacio negativo para máxima cercanía */
  row-gap: 6px;
  justify-items: center; /* Puedes usar start/end/stretch según lo que prefieras */
  align-items: center;
}
...
/* En móviles, ajusta el ancho aún más */
@media (max-width: 640px) {
  .stamp-icon {
    width: 65%;
  }
  ...
  .icon-grid-tight {
    column-gap: -14px !important; /* más negativo en móviles */
  }
}
...
/* Clase auxiliar para espaciado entre columnas ultra pegado */
.col-gap-tighter {
  column-gap: -60px !important;
}

/* En móviles, aún más pegadas */
@media (max-width: 640px) {
  .col-gap-tighter {
...
const order = [
  ...
  else { delete libro.dataset.realm; }  // en Home no hay marco
}
